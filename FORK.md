...
| `cmake/resources/generate_resources.cmake` | ICON resource must be on the exe, not the OBJECT lib | stop appending `windows.rc` to `RES_AUTOGEN_FILES` (paired with `src/CMakeLists.txt` `target_sources` on the executable) | Low |
...
| `src/widgets/dialogs/UserInfoPopup.cpp` / `.hpp` | usercard gets a "Name history" option + a GQL-extras label | one include + one `LabelButton` + connect (pre-existing); batch U2: one include + one `LimerinoUserCardWidget` + one `setTarget()` call inside the existing Helix-success lambda (no `ui_` struct changes); batch F1: name-history dialog opened with `nullptr` parent (popup auto-close used to destroy the dialog mid-fetch → UAF crash); batch F4: connect `extrasChanged` → re-append sub suffix (IVR/GQL race fix) + `.hpp` `subageBaseText_` member; crossban: one include + one `LabelButton` "Crossban" + connect opening `LimerinoCrossbanDialog` with `nullptr` parent; E9: Artist/Unartist/Lead mod use `PixmapButton` + `buttons/{artist,unartist,leadmod}.png` (30x30, tooltips) matching mod/unmod; A: "View user editors (7TV)" passes `userId_`/`userName_` (not channel `roomId`) | Low |
| `src/widgets/splits/SplitHeader.cpp` / `.hpp` | split menus + mod toolbar get fork actions | "Follow channel" + "View followers/following" menu lines (batch 2); predictions button member + creation + layout slot + vis hints in `updateIcons`/`updateAddButtonMargins` (batch 4); "Filter events..." conditional menu entry for `/events` + 2 hook includes (live-updates P0); "Nuke messages..." conditional menu entry gated on `hasModRights() && isTwitchOrKickChannel()` opening `LimerinoNukeDialog` + 1 include (batch N3); E6: moved View followers/following from chat-mode menu into the split three-dots menu beside Follow channel, still own-channel + Limerino auth gated | Low |
...
| `src/singletons/Settings.hpp` | register `/highlighting/groups` store | one include + `ChatterinoSetting<std::vector<HighlightGroup>>` + `SignalVector<HighlightGroup> highlightGroups`; batch N4 added `limerinoNukePresets` (`QStringSetting`, presets as JSON); batch N5 added `limerinoAutoActions` (`QStringSetting`, rules as JSON); crossban added `limerinoCrossbanPresets` + `limerinoCrossbanLastPresetId`; E7 added `limerinoChatColorRecents` + `limerinoChatColorLast`; B4.1 added `limerinoPubSubDedupeEnabled` + `Settings::hasInstance()`; theme recents added `limerinoThemeRecents` | Low |
...
| `src/controllers/highlights/BadgeHighlightModel.cpp` | same for badge highlights; E4: clear default checkable on Group cells | one line + Group cell flags | Low |
...
| `src/controllers/highlights/HighlightModel.cpp` | populate Group cell / read groupId back from it / blank it on the 11 pinned rows | three blocks in getRowFromItem/getItemFromRow/afterInit; F3: ctor connects `highlightGroups.delayedItemsChanged` → `refreshGroupCells()` (rebuilds every non-custom row's Group option roles + re-derives its display name); E4: clear default `ItemIsUserCheckable` on Group cells so the column does not paint a checkbox before the name | Low |
| `src/controllers/highlights/UserHighlightModel.cpp` | identical handling for the users table | mirrors HighlightModel (shared Column enum) + SelfMessageRow blanking; F3: same ctor hook + `refreshGroupCells()`; hpp gains the holder + method decl; E4: same Group-cell flag clear | Low |
...
| `src/limerino/` (incl. `src/limerino/LimerinoPage.{hpp,cpp}`, Limerino settings page; E5: `LimerinoAutoActionsPage.{hpp,cpp}` owns the Auto Actions list/editor moved off LimerinoPage; list reports a compact sizeHint + AsNeeded horizontal scrollbar so long actions do not widen Settings) | fork setup / Limerino tab |
...
| `src/providers/limerino/gql/` (`LimerinoGql`, `PersistedQueries`) | GQL client (persisted + inline) with central errors[] extraction; query/hashes transcribed from `pluginforreference/requests.lua`; `executeInlineAllowPartial` keeps `data` when sibling field errors exist (usercard); E10: `SEVENTV_ONE_USER_QUERY` selects `editor.id` (not `UserEditor.id`) |
...
| `src/widgets/dialogs/limerino/LimerinoUserCardWidget.{hpp,cpp}` | per-usercard extras label (language tag top-right in the header box; team + sub rows later); QPointer + request-generation teardown (batch U2); batch F4: unknown platform strings render raw instead of hidden, SKU chip, `extrasChanged` signal; E8: settings-failed tooltip placeholder; gift-from-name in sub suffix; E8.1: language-only show no longer gated on `QWidget::isVisible()` (hidden parent after refetch made tag appear only when a team also forced show) |
| `src/providers/limerino/commands/` (`LimerinoCommands.*`) | ported-command registration entry (batches 1-12); A: `Roles` `showSeventvUserEditors` takes usercard Twitch id (not channel roomId) |
...
| `src/providers/limerino/pubsub/HermesChannelTopics.{hpp,cpp}` | raid/polls/predictions channel topics (batch P1); B5: `raid_go_v2`; B4.2: source resolve + target_display_name; B6: poll choices |
| `src/providers/limerino/pubsub/HermesUserTopics.{hpp,cpp}` | authenticated user topics incl. chatrooms-user-v1 warn/ack flow (batch P2); E1.c: `channel_banned_alias_restriction_update` |
| `src/limerino/PubSubEventsChannel.{hpp,cpp}` | `/events` special channel + per-event-type filter storage (batch P0; renamed from `/pubsub-events` in F5); batch F6: events render as structured messages (themed category chip, plain text, raw topic+payload in chip tooltip, optional #channel jump link); E1.d: compact raw store + message id; unread tab colour: do not set `DoNotTriggerNotification` |
...
| `src/providers/limerino/theme/LimerinoThemeGenerator.{hpp,cpp}` | seed + real base JSON: `recolorTheme` replaces only matching seed RGB; Dark/Light/Black/White.json are the bases (synthesized leaves only if qrc missing) |
| `tests/src/LimerinoTheme.cpp` | generator tests: leaf inventory, styleSheet exclusion, public Limerino `$schema` URL, iconTheme inversion, determinism, low-contrast flagging, transparent literal; recolor keeps unrelated leaves (link/alternate) |
| `src/providers/limerino/theme/LimerinoThemeStore.{hpp,cpp}` | two-file-model seed wrapper (`limerinoThemeVersion`), filename sanitization, Themes/ install + rescan + select; full-theme files imported as-is; MRU recents (`limerinoThemeRecents`) |
| `tests/src/LimerinoThemeStore.cpp` | seed serde round-trip, version gate, filename sanitizer (batch T2) |
| `src/widgets/dialogs/limerino/LimerinoThemeDialog.{hpp,cpp}` | theme creator UI: four-color seed editor over a real base JSON, Recents combo, Dark/Light/Black/White.json buttons, shared `LimerinoColorField`, live preview via Themes/_LimerinoPreview.json |
...
| `LimerinoThemeGenerator` | `recolorTheme(base, from, to)` over real Dark/Light/Black/White.json; `$schema` is the public raw URL; synthesize only if qrc missing |
| `LimerinoThemeStore` | `limerino_theme.json` seed wrapper, filename sanitize, Themes/ install + recents MRU (`/limerino/theme/recents`) |
| `LimerinoThemeDialog` | Recents combo + Dark/Light/Black/White.json bases; four seed colours; live preview via `Themes/_LimerinoPreview.json` |
