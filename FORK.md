...
| `src/widgets/dialogs/UserInfoPopup.cpp` / `.hpp` | usercard gets a "Name history" option + a GQL-extras label | one include + one `LabelButton` + connect (pre-existing); batch U2: one include + one `LimerinoUserCardWidget` + one `setTarget()` call inside the existing Helix-success lambda (no `ui_` struct changes); batch F1: name-history dialog opened with `nullptr` parent (popup auto-close used to destroy the dialog mid-fetch → UAF crash); batch F4: connect `extrasChanged` → re-append sub suffix (IVR/GQL race fix) + `.hpp` `subageBaseText_` member; crossban: one include + one `LabelButton` "Crossban" + connect opening `LimerinoCrossbanDialog` with `nullptr` parent; E9: Artist/Unartist/Lead mod use `PixmapButton` + `buttons/{artist,unartist,leadmod}.png` (30x30, tooltips) matching mod/unmod | Low |
